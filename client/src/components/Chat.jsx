...
import { sally, auth, upload } from '../lib/api.js'
    ...
    <div className="row" style={{gap:12}}>
      ...
      <div className="card" style={{width:320, height:520, display:'flex', flexDirection:'column'}}>
        <div style={{display:'flex', gap:8, alignItems:'center', marginBottom:8}}>
          <h3 style={{margin:0, flex:1}}>Messaging</h3>
        </div>
        <input placeholder="Search" value={filter} onChange={e=>setFilter(e.target.value)} style={{marginBottom:8}} />
      ...
      <div className="card" style={{flex:1, maxWidth:820, height:520, display:'flex', flexDirection:'column'}}>
        <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', borderBottom:'1px solid #eee', paddingBottom:8}}>
          <h3 style={{margin:0}}>{withUser ? `Chat with @${withUser}` : 'Select a conversation'}</h3>
          {typingPeer ? <span style={{fontSize:12, fontWeight:400, color:'#666'}}>typing</span> : null}
          <div style={{flex:1}} />
          <div className="row" style={{alignItems:'center', gap:8}}>
            <div className="col" style={{width:180}}>
              <input placeholder="@friend" value={withUser} onChange={e=>setWithUser(e.target.value)} />
            </div>
        ...
        <div className="row" style={{marginTop:8, alignItems:'center', gap:8}}>
          ...
          <div><button className="primary" onClick={send} disabled={!withUser || (!text.trim() && files.length===0)}>Send</button></div>
        </div>
      </div>
    </div>
  )
